...
import { MyDogWalk } from "../../api/MyPage/MyDogWalk";

function MyDogWalkList({ route }) {
  const dogId = route.params.dogId;

  const [dogWalks, setDogWalks] = useState([]);

  const fetchDogWalks = async () => {
    try {
      const res = await MyDogWalk(dogId);
      if (res) {
        setDogWalks(res.data);
      }
    } catch (error) {
      console.error("error fetching dogWalks", error);
    }
  };
